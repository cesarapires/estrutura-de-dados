...
unsigned hashEA::getCapacidade() {
    return capacidade;
}

dado* hashEA::getVetDados() {
    return vetDados;
}
      

void desfragmentar(hashEA& tabela) {
    hashEA novaTabela(tabela.getCapacidade()); // Cria uma nova tabela hash com a mesma capacidade da tabela original

    for (unsigned i = 0; i < tabela.getCapacidade(); i++) {
        if (tabela.getVetDados()[i] != INVALIDO && tabela.getVetDados()[i] != REMOVIDO) {
            novaTabela.inserir(tabela.getVetDados()[i].assunto, tabela.getVetDados()[i].nomeInteressado, tabela.getVetDados()[i].tipo, tabela.getVetDados()[i].numeroProcesso);
        }
    }

    tabela = novaTabela; // Substitui a tabela original pela nova tabela desfragmentada
}
